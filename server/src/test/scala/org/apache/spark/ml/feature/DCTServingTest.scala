...
object DCTServingTest {
  ...
  def trans(model: DCT): SDFrame = {
    val transModel = ModelUtils.transTransformer(model).asInstanceOf[DCTServing]
...
//    }
    ...
    data.put(model.getInputCol, Vectors.dense(0.0, 1.0, -2.0, 3.0))
