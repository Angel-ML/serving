...
object PCAServingModelTest {
  ...
  def trans(model: PCAModel): SDFrame = {
    val transModel = ModelUtils.transModel(model).asInstanceOf[PCAServingModel]
...
//    }
    ...
    data.put(model.getInputCol,  Vectors.sparse(5, Seq((1, 1.0), (3, 7.0))))
