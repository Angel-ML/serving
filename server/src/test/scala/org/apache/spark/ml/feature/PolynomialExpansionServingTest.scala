...
object PolynomialExpansionServingTest {
  def main(args: Array[String]): Unit = {
    ...
    val res = trans(polyExpansion)
    println(res.schema, res.columns.length, res.columns(0),
      res.getRow(0).get(0).toString, res.getRow(0).get(1).toString)
    res.printSchema()
  }

  def trans(model: PolynomialExpansion): SDFrame = {
    val transModel = ModelUtils.transTransformer(model).asInstanceOf[PolynomialExpansionServing]
...
//    }
    ...
    data.put(model.getInputCol, Vectors.dense(2.0, 1.0))
