...
object VectorIndexerServingModelTest {
  def main(args: Array[String]): Unit = {
    ...
    val res = trans(indexerModel)
    println(res.schema, res.columns.length, res.columns(0),
      res.getRow(0).get(0).toString, res.getRow(0).get(1).toString)
    res.printSchema()
  }

  def trans(model: VectorIndexerModel): SDFrame = {
    val transModel = ModelUtils.transModel(model).asInstanceOf[VectorIndexerServingModel]
